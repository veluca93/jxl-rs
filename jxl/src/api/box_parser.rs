...
use super::{JxlBitstreamInput, JxlSignatureType, check_signature_internal, process::SmallBuffer};
...
impl BoxParser {
        ...
        loop {
            match self.state.clone() {
                ...
                ParseState::SkippableBox(mut s) => {
                    ...
                    let skipped = if self.box_buffer.len() > 0 {
                        let c = self.box_buffer.len().min(num);
                        self.box_buffer.consume(c);
                        c
                    } else {
                        input.skip(num)?
                    };
    ...
    fn consume_codestream(&mut self, amount: u64) {
