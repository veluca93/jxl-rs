...
use crate::{
    ...
    var_dct::{
        dct::{
            DCT1D, DCT1DImpl, IDCT1D, IDCT1DImpl, MAX_SCRATCH_SPACE, compute_scaled_dct, dct2d,
            idct2d,
        },
...
macro_rules! test_idct1d_eq_slow_n {
    ($test_name:ident, $n_val:expr, $tolerance:expr) => {
        ...
        fn $test_name() {
            ...
            for i in 0..N * M {
                assert_almost_abs_eq(output[i * M], output_matrix_slow[i][0] as f32, $tolerance);
            }
        }
    };
}
    ...
    for r_idx in 0..N {
        for c_idx in 0..M {
            assert_almost_abs_eq(
                output_fast_impl[r_idx * M + c_idx],
